# Puts the Erlang/Gleam toolchain and rebar3 on PATH. Source this, don't run it.
...
if [ -z "$rebar3_path" ]; then
  echo "toolchain-path.sh: no rebar3 found under $mix_home after mix local.rebar" >&2
  exit 1
fi
