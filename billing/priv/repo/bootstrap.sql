...
-- `billing` role itself has no privilege to create roles or schemas),
-- before `mix ecto.migrate` (or `mix billing.bootstrap && mix
-- ecto.migrate`) can create `billing`'s tables inside schema `billing`.
--
-- Run it either:
--   * by hand:      psql -U postgres -d baalbek -f priv/repo/bootstrap.sql
--   * or scripted:  mix billing.bootstrap
--     (lib/mix/tasks/billing.bootstrap.ex reads this exact file and
...
-- containing only ";" — see lib/mix/tasks/billing.bootstrap.ex's header
...
-- shared across the whole database, not scoped to `billing`'s own role/
-- schema: `CREATE ROLE` writes the cluster-wide `pg_authid` catalog, and
-- `REVOKE ALL ON SCHEMA public FROM billing` is a read-modify-write of
-- the *same* `pg_namespace` row for `public` that identity's (or any
-- other component's) bootstrap also read-modify-writes for its own role.
-- Two sessions racing on that path is exactly Postgres's well-known
...
EXCEPTION WHEN duplicate_object THEN
  RAISE NOTICE 'role "billing" already exists, skipping';
END
$$
;

-- Every session opened as `billing` resolves unqualified table names
-- inside schema `billing` without needing `search_path` set
-- per-connection by Ecto — this is what lets Billing.Repo's
...
-- `billing` owning its own schema already implies full rights inside it;
...
GRANT USAGE, CREATE ON SCHEMA billing TO billing
;

ALTER DEFAULT PRIVILEGES IN SCHEMA billing GRANT ALL ON TABLES TO billing
;
...
REVOKE ALL ON SCHEMA public FROM billing
;
...
-- mix billing.bootstrap, or a one-shot `psql -f`) then closes anyway,
