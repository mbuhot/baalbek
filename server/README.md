...
- **Real Mix path deps on all five domain apps** (`core`, `identity`,
  ...
  repo that takes them. Only `core` declares an HTTP surface; the others are
  real build-time dependencies regardless, and a future extension can expose
  `identity` and `billing` too.
- **`core`'s JSON:API**, mounted, not restated. Each `core` resource carries
  an `AshJsonApi.Resource` `json_api do routes do ... end end` block naming
  its own paths and actions, and `Core` carries `AshJsonApi.Domain`.
  `ServerWeb.JsonApiRouter` is `use AshJsonApi.Router, domains: [Core]` — the
  Plug that serves whatever those resources declare. `server` holds no
  resource definitions and opens no database connection of its own.
...
- **Cross-app `boundary` enforcement turns on**: `mix.exs` sets
  ...
  `Core.Data.Repo` or `PricingNative.Native` directly).
- **ExUnit** (`test/server_web/`): that the mounted surface answers over real
  HTTP through `ServerWeb.Endpoint`. The per-resource JSON:API contract is
  asserted in `core`'s own suite, against the resources that declare it.
