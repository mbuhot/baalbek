...
defmodule MoonElixirPlugin do
  ...
  def main(specs) do
    root = File.cwd!()
    projects = Enum.map(specs, &read(&1, root))
  ...
  defp read(spec, root) do
    ...
    case evaluate(Path.expand(source, root), root) do
      {:error, message} ->
        failed(id, message)

      {:ok, deps, third_party} ->
        %{"id" => id, "error" => nil, "deps" => deps, "third_party" => third_party}
    end
  end
...
pub struct ProjectManifest {
    pub id: String,
    /// Mix's own message when this project's manifest could not be evaluated.
    /// Every list is then empty and nothing is inferred for the project.
